...
namespace Anw.HttpUtils.CsharpHttpHelper.Helper
{
    internal class HtmlHelper
    {
            ...
            if (Regex.IsMatch(html, alist, RegexOptions.IgnoreCase))
            {
                ...
                foreach (Match match in Regex.Matches(html, alist, RegexOptions.IgnoreCase))
                {
                    ...
                    try
                    {
                        ...
                        var imgList = GetImgList(aItem.Text);
        ...
        internal static List<ImgItem> GetImgList(string html)
        {
            ...
            var imgList = RegexString.ImgList;
        ...
        {
            html = Regex.Replace(html, RegexString.Nscript, string.Empty,
        ...
        {
            return Regex.Replace(html, RegexString.NewLine, string.Empty,
                RegexOptions.IgnoreCase | RegexOptions.Compiled);
        }

        internal static string GetBetweenHtml(string html, string s, string e)
        {
            var pattern = string.Format("{0}{1}{2}", s, RegexString.AllHtml, e);
            string result;
